 ...
 * A singleton for storing and accessing 
 * all <code>Member</code> instances.  
 * The class assures each member has a unique
 * id and is responsible for serializing data about members.
 * 
 * updated 3/17/2023
 */
public class MemberList implements Iterable<Member>, Serializable {
	...
	public Member search(int memberId) {
		for (Iterator<Member> iterator = 
				members.iterator(); iterator.hasNext();) {
	...
	public boolean addMember(Member member) {
		//System.out.println("Line 50, Memberist.addMember(): memberId = " + 
			//member.getId() + ": memberName = ");
	...
	public boolean removeMember(int memberId) {
		//System.out.println("Line 57, Memberist.addMember(): " + 
			//"memberId = " + memberId);
		for (Iterator<Member> iterator = members.iterator(); 
				iterator.hasNext();) {
			Member member = iterator.next();
			//System.out.println("Line 60 member.getId = " + 
				//"member.getId() + ", memberId = " + memberId);
			if (member.getId() == memberId) {
